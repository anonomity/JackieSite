...
const makeTypeName = type => _lodash.default.upperFirst(_lodash.default.camelCase(`${typePrefix} ${type}`));
...
const getLocalizedField = ({
  ...
  if (!_lodash.default.isUndefined(field[locale.code])) {
    return field[locale.code];
  } else if (!_lodash.default.isUndefined(locale.code) && !_lodash.default.isUndefined(localesFallback[locale.code])) {
...
const buildFallbackChain = locales => {
  ...
  _lodash.default.each(locales, locale => localesFallback[locale.code] = locale.fallbackCode);
...
function prepareTextNode(id, node, key, text) {
  ...
  const textNode = {
    ...
    [key]: str,
    internal: {
      type: _lodash.default.camelCase(`${node.internal.type} ${key} TextNode`),
...
function prepareJSONNode(id, node, key, content) {
  ...
  const JSONNode = { ...(_lodash.default.isPlainObject(content) ? { ...content
    } : {
      content: content
    }),
    id,
    ...
    children: [],
    internal: {
      type: _lodash.default.camelCase(`${node.internal.type} ${key} JSONNode`),
...
const createNodesForContentType = ({
  ...
  locales.forEach(locale => {
    ...
    const entryNodes = entries.map(entryItem => {
      ...
      const entryItemFields = _lodash.default.mapValues(entryItem.fields, (v, k) => {
      ...
      Object.keys(entryItemFields).forEach(entryItemFieldKey => {
        ...
        if (fieldType === `Text`) {
          ...
          delete entryItemFields[entryItemFieldKey];
        } else if (fieldType === `RichText` && _lodash.default.isPlainObject(entryItemFields[entryItemFieldKey])) {
        ...
        } else if (fieldType === `Object` && _lodash.default.isPlainObject(entryItemFields[entryItemFieldKey])) {
          ...
          delete entryItemFields[entryItemFieldKey];
        } else if (fieldType === `Object` && _lodash.default.isArray(entryItemFields[entryItemFieldKey])) {
