...
const {
  selectAll
} = require(`unist-util-select`); // TODO(v5): use gatsby/sharp
...
module.exports = async ({
  ...
  const markdownImageNodes = selectAll(`image`, markdownAST); // This will also allow the use of html image tags
  ...
  const generateImagesAndUpdateNode = async function (node) {
    ...
    const metaReader = sharp(); // @todo to increase reliablility, this should use the asset downloading function from gatsby-source-contentful
    ...
    try {
      ...
      console.log(error);
      reporter.panic(`The image "${node.url}" (with alt text: "${node.alt}") doesn't appear to be a supported image format.`, error);
    }
